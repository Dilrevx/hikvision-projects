...
def parse_common_header(f: BinaryIO):
    ...
    info(f"magic: {header_magic:x} checksum: {header_checksum:x} size: {header_size} count: {file_count}")
...
def parse_hk30(f: BinaryIO, header_key: int, file_key: int, save_folder: str | None = None):
...
while True:
    ...
    magic = int.from_bytes(next_header[:4], "little")
    match magic:
        case 0x484B5753:
